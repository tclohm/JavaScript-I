...
console.log(`The last car is make: ${lastCar.car_make}, model: ${lastCar.car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console

let sortCarModel = function(arr) {
  let result = [];
  for(let i = 0 ; i < arr.length ; i++) {
...
// less code to sort car_models
let carModelsSorted = inventory.map( (carObject) => {
    return carObject.car_model;
}).sort();

// console.log(carModelsSorted(inventory[0], inventory.splice(1,inventory.length), 0));

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let getCarYears = function(arr) {
  let result = [];
  for(let i = 0; i < arr.length; i++) {
...
// less code to create a new array for carYears
let carYears = inventory.map ( (carObject) => {
  return carObject.car_year;
});

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.

let getOldCars = function(arr) {
  let counter = 0;
  for(let i = 0; i < arr.length; i++) {
    if(arr[i] < 2000) {
      counter += 1;
...
console.log(getOldCars(getCarYears(inventory)));


// less code to filter through all the cars that are older than 2000
let oldCars = carYears.filter( (carYears) => {
  if (carYears < 2000) { return carYears; }
});
...
let getBMWAndAudi = function(arr) {
  let result = [];
  for(let i = 0; i < arr.length; i++) {
...
// less code for filter and find BMW's and Audi's
let BMWAndAudi = inventory.filter( (carObject) => {
